...
def random_short_side_scale_jitter_list(
    images, min_size, max_size, boxes=None, random_param=None
):
    """
    ...
    if random_param is not None:
        size = int(
            round(
                1.0
                / ((1.0 / min_size - 1.0 / max_size) * random_param + 1.0 / max_size)
            )
        )
    else:
...
def horizontal_flip_list(prob, images, order="CHW", boxes=None, random_param=None):
    """
    ...
    if (random_param if random_param is not None else np.random.uniform()) < prob:
        if boxes is not None:
...
def random_crop_list(
    images, size, pad_size=0, order="CHW", boxes=None, random_param=None
):
    """
    ...
    if order == "CHW":
        ...
        if height > size:
            if random_param is not None:
                y_offset = int((height - size + 1) * random_param[0])
            else:
                y_offset = int(np.random.randint(0, height - size))
        x_offset = 0
        if width > size:
            if random_param is not None:
                x_offset = int((width - size + 1) * random_param[1])
            else:
        ...
        if height > size:
            if random_param is not None:
                y_offset = min(
                    int((height - size + 1) * random_param[0]), height - size
                )
            else:
                y_offset = int(np.random.randint(0, height - size))
        x_offset = 0
        if width > size:
            if random_param is not None:
                x_offset = min(int((width - size + 1) * random_param[1]), width - size)
            else:
                x_offset = int(np.random.randint(0, width - size))
        cropped = [
            image[y_offset : y_offset + size, x_offset : x_offset + size, :]
