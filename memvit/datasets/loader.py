...
from memvit.datasets.multigrid_helper import ShortCycleBatchSampler
...
def construct_loader(cfg, split, is_precise_bn=False):
    ...
    if isinstance(dataset, torch.utils.data.IterableDataset):
        loader = torch.utils.data.DataLoader(
            ...
            if (split == "test" and cfg.DATA_LOADER.TEST_NUM_WORKERS >= 0)
            else cfg.DATA_LOADER.NUM_WORKERS,
...
def shuffle_dataset(loader, cur_epoch):
    ...
    assert isinstance(
        sampler, (RandomSampler, DistributedSampler, SequentialSampler)
...
class SplitClipLoader:
    def __init__(self, loader, num_splits):
        self._loader = loader
    ...
    def __iter__(self):
        for data in self._loader:
            assert len(data[0]) == 1
            for data_chunck in data[0][0].split(
                data[0][0].shape[2] // self._num_splits, dim=2
            ):
                yield ([data_chunck], data[1], data[2], data[3])
