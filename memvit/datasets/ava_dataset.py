...
class Ava(torch.utils.data.Dataset):
    ...
    def _load_data(self, cfg, cur_epoch=0):
        """
        ...
        ) = ava_helper.get_keyframe_data(boxes_and_labels, random_seed=cur_epoch)
        ...
        if self.online_mem:
            self.real_num_videos = len(self._keyframe_indices)
    ...
    def sample_per_video_aug_params(self):
        video_ids = set([x[0] for x in self._keyframe_indices])
        self.per_video_aug_params = {
            video_id: (
                random.random(),
                random.random(),
                random.random(),
    ...
    def _images_and_boxes_preprocessing_cv2(self, imgs, boxes, random_params=None):
        """
    ...
    def __getitem__(self, idx):
        ...
        if self.online_mem:
            idx = (
                (idx // self.num_streams) + self.num_iters * (idx % self.num_streams)
        ...
        if self.cfg.DATA.PERTURB_AVA_FRAMES and (self._split == "train"):
            new_seq = []
            for seq_i, frame in enumerate(seq):
                start = -self.cfg.DATA.SAMPLING_RATE // 2
                end = self.cfg.DATA.SAMPLING_RATE // 2
                if self.cfg.MEMVIT.ENABLE:
                    if seq_i == 0:
                        start = 0
                    elif seq_i == len(seq) - 1:
                        end = 0
                new_seq.append(frame + np.random.randint(start, end))
            seq = new_seq
