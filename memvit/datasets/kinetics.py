...
class Kinetics(torch.utils.data.Dataset):
    ...
    def __init__(self, cfg, mode, num_retries=10):
        ...
        self.mode = mode
        self.cfg = cfg
        self.online_mem = cfg.MEMVIT.ENABLE
        ...
        if self.online_mem:
            if self.mode in ["train", "val"]:
                self._num_clips = cfg.MEMVIT.K400_NUM_CLIPS_TRAIN
            else:
                self._num_clips = cfg.MEMVIT.K400_NUM_CLIPS_TEST
    ...
    def _construct_loader(self, random_seed=0):
        """
        ...
        self._om_sampling_start = {}
        with g_pathmgr.open(path_to_file, "r") as f:
            all_lines = list(f.read().splitlines())
            if self.online_mem:
                random.seed(random_seed)
                random.shuffle(all_lines)
            for clip_idx, path_label in enumerate(all_lines):
                ...
                if self.online_mem:
                    self._om_sampling_start[full_path] = random.random()
        assert (
        ...
        if self.online_mem:
            self.real_num_videos = len(self._path_to_videos)
            self.num_streams = self.cfg.NUM_SHARDS * (
                self.cfg.TEST.BATCH_SIZE
                if self.mode == "test"
                else self.cfg.TRAIN.BATCH_SIZE
            )
            self.num_iters = int(np.ceil(self.real_num_videos / self.num_streams))
            logger.info(
                f"streaming: real_num_videos: {self.real_num_videos}, "
                f"num_streams: {self.num_streams}, "
                f"num_iters: {self.num_iters}"
            )

            if self.mode == "test":
                # Padding videos. TODO: fix this.
                num_pad = self.num_iters * self.num_streams - self.real_num_videos
                for idx in range(
                    len(self._path_to_videos),
                    len(self._path_to_videos) + num_pad,
                ):
                    self._video_meta[idx] = {}

                self._path_to_videos = (
                    self._path_to_videos + self._path_to_videos[:num_pad]
                )
                self._labels = self._labels + self._labels[:num_pad]
                self._spatial_temporal_idx = (
                    self._spatial_temporal_idx + self._spatial_temporal_idx[:num_pad]
                )
    ...
    def __getitem__(self, index):
        ...
        if self.online_mem:
            index = (
                (index // self.num_streams)
                + self.num_iters * (index % self.num_streams)
            ) % self.real_num_videos
        if self.mode in ["train", "val"]:
        ...
        for i_try in range(self._num_retries):
            ...
            om_sampling_start = None
            if self.online_mem:

                if self.mode in ["train", "val"]:
                    om_sampling_start = self._om_sampling_start[
                        self._path_to_videos[index]
                    ]
                else:
                    om_sampling_start = 0.0
            ...
            frames = decoder.decode(
                ...
                om_sampling_start=om_sampling_start,
                num_om_clips=self._num_clips,
            )
            ...
            if self.aug:
                if self.cfg.AUG.NUM_SAMPLE > 1:
                    ...
                    for _ in range(self.cfg.AUG.NUM_SAMPLE):
                        ...
                        path_list.append(self._path_to_videos[index])
                    return (
                        frame_list,
                        label_list,
                        index_list,
                        {"video_name": path_list},
                    )
