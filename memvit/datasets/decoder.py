...
def get_start_end_idx(
    ...
    """
    if om_sampling_start is None:
        delta = max(video_size - clip_size, 0)
    else:
        delta = max(video_size - clip_size * num_om_clips, 0)

    if om_sampling_start is not None:
        start_idx = min(
            math.floor(delta * om_sampling_start) + clip_idx * clip_size,
            video_size - 1,
        )
    elif clip_idx == -1:
    ...
    # logger.info(f'clip_idx: {clip_idx}, start: {start_idx}, end: {end_idx}, video_size: {video_size}, delta {delta} om {om_sampling_start}')
