...
def get_keyframe_data(boxes_and_labels, random_seed=None):
    """
    ...
    count = 0
    video_indices = list(range(len(boxes_and_labels)))
    if random_seed is not None:
        logger.info(f"Shuffle AVA video order (seed = {random_seed})")
        random.seed(random_seed)
        random.shuffle(video_indices)
    for video_idx in video_indices:
        sec_idx = 0
        keyframe_boxes_and_labels[video_idx] = []
