...
import memvit.utils.multiprocessing as mpu
import numpy as np
import psutil
import torch
...
from memvit.datasets.utils import pack_pathway_output
from memvit.models.batchnorm_helper import SubBatchNorm3d
...
def get_model_stats(model, cfg, mode, use_train_input):
    ...
    if cfg.MEMVIT.ENABLE:
        inputs = (inputs[0], ["dummy_video_name"])
        for i in range(10):
            logger.info(f"Online memory flop calculation run {i}...")
