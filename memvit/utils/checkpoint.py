...
from memvit.utils.c2_model_loading import get_name_convert_func

try:
    from iopath.fb.manifold import ManifoldPathHandler
    from memvit.utils.env import checkpoint_pathmgr as pathmgr
except:
    pass
...
def inflate_weight(state_dict_2d, state_dict_3d):
    ...
    for k, v2d in state_dict_2d.items():
        ...
        if len(v2d.shape) == 4 and len(v3d.shape) == 5:
            ...
            v3d = v2d
        elif len(v2d.shape) == 5 and len(v3d.shape) == 5 and v2d.shape[2] == 1:
            logger.info("Inflate {}: {} -> {}: {}".format(k, v2d.shape, k, v3d.shape))
            # Dimension need to be match.
            assert v2d.shape[-2:] == v3d.shape[-2:]
            assert v2d.shape[:2] == v3d.shape[:2]
            v3d = v2d.repeat(1, 1, v3d.shape[2], 1, 1) / v3d.shape[2]
        else:
            logger.info(
...
def load_checkpoint(
    ...
    if convert_from_caffe2:
        ...
        try:
            pathmgr.register_handler(ManifoldPathHandler(read_chunk_size=200000000))
        except:
            pass
        ...
        if inflation:
            ...
            for k, v in pre_train_dict.items():
                if k.endswith(".rel_pos_t") or k.endswith(".rel_pos_mem_t"):
...
def load_train_checkpoint(cfg, model, optimizer, scaler=None):
    ...
    if cfg.TRAIN.AUTO_RESUME and has_checkpoint(cfg.OUTPUT_DIR):
        ...
        checkpoint_epoch = load_checkpoint(
            ...
            model,
            cfg.NUM_GPUS > 1,
            None if cfg.TRAIN.CHECKPOINT_NO_LOAD_OPT else optimizer,
            scaler=scaler,
