...
from memvit.utils.ava_evaluation import object_detection_evaluation, standard_fields
...
def read_csv(csv_file, class_allowlist=None, load_score=False):
    """Loads boxes and class labels from a CSV file in the AVA format.
    CSV file format described at https://research.google.com/ava/download.html.
    Args:
      csv_file: A file object.
      class_allowlist: If provided, boxes corresponding to (integer) class labels
    ...
    with g_pathmgr.open(csv_file, "r") as f:
        ...
        for row in reader:
            ...
            action_id = int(row[6])
            if class_allowlist and action_id not in class_allowlist:
                continue
            score = 1.0
            if load_score:
                score = float(row[7])
...
def evaluate_ava_from_files(labelmap, groundtruth, detections, exclusions):
    ...
    categories, class_allowlist = read_labelmap(labelmap)
    excluded_keys = read_exclusions(exclusions)
    groundtruth = read_csv(groundtruth, class_allowlist, load_score=False)
    detections = read_csv(detections, class_allowlist, load_score=True)
...
def get_ava_eval_data(
    ...
    for i in range(scores.shape[0]):
        ...
        for cls_idx, score in enumerate(one_scores):
            if cls_idx + 1 in class_allowlist:
